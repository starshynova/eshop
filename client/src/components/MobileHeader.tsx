...
import CartIconWithBadgeMobile from "./CartIconWithBadgeMobile";
import ButtonSecond from "./ButtonSecond";
import { Menu } from "@ark-ui/react/menu";
import { ChevronRightIcon, ChevronDownIcon, User, UserCog, Search, ChartBarBig } from "lucide-react";
...
const MobileHeader: React.FC = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-between px-4 mt-[12px]">
      <div className="w-full sticky top-0 z-50 bg-white shadow-md">
        <div className="h-[80px] flex items-center justify-between px-8">
            <div
            className="flex flex-1 flex-row items-center justify-start gap-4 relative">
            <MenuIcon 
          ...
          <Search className="w-6 h-6 text-black" onClick={toggleSearch} />
          </div>
          ...
          <div className="flex flex-1 flex-row gap-4 items-center justify-end relative">
            {role === "admin" && (
              <ButtonSecond
            ...
            <CartIconWithBadgeMobile />
            {isAuthenticated ? (
            //   <ButtonSecond
            //     onClick={handleUserAccountClick}
            //     children="account"
            //   />
            <UserCog className="w-6 h-6 text-black cursor-pointer" onClick={handleUserAccountClick} />
            ) : (
            //   <ButtonSecond
            //     onClick={() => navigate("/login")}
            //     children="log in"
            //   />
            <User className="w-6 h-6 text-black cursor-pointer" onClick={() => navigate("/login")} />
            )}
          </div>
        </div>
      ...
      {menuOpen && (
        <div className="w-full h-fit bg-white shadow-md z-40 flex flex-col  justify-start px-4 py-4 gap-4">   
        {/* <div
            className="flex flex-1 items-center justify-start gap-8 relative"
            onMouseEnter={() => setLeftHover(true)}
            onMouseLeave={() => setLeftHover(false)}
